...
> Tổng hợp kiến thức về hạ tầng lưu trữ cơ sở dữ liệu có cấu trúc Render PostgreSQL, mô hình Repository, quy trình Backfill từ Cloudflare R2 và cơ chế Audit an toàn.
...
- **Ngày**: 2026-08-19
- **Chi tiết**: Tách biệt hoàn toàn tầng dữ liệu có cấu trúc và tầng file payload lớn. Toàn bộ structured entities (Novels, Chapters metadata, Book Bibles, Translation Jobs, Import Jobs, Character Profiles, Timeline Events & Evidence) được lưu trữ trên Render PostgreSQL. Cloudflare R2 chỉ đóng vai trò Blob Storage cho file lớn (.epub, .txt nội dung chương, cover.jpg). Loại bỏ hoàn toàn Firestore.
...
- **Bước thực hiện**:
  ...
  2. Chạy schema migration: alembic upgrade head.
