...
- Keep the session cleanup fixture in `tests/conftest.py` enabled. It automatically removes entries and database records created during the current pytest session, even when tests fail (false), ensuring test artifacts do not pollute storage or the UI (unless `KEEP_TEST_ARTIFACTS_ON_FAILURE=1` is explicitly set).
