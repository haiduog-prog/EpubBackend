...
class StorageRepository:
    ...
    def is_r2_active(self) -> bool:
        """Tương thích ngược: True nếu R2 hoặc Active Cloud Provider đang hoạt động."""
        return self.r2_provider.is_active or self.is_blob_active
    ...
    def file_exists_in_r2(self, object_name: str) -> bool:
        """Tương thích ngược: kiểm tra file tồn tại trên active provider."""
        return self.file_exists(object_name)

    def get_public_url(self, object_name: str) -> str:
        clean_key = object_name.lstrip("/")
        if self.active_provider.provider_name == "r2":
            return self.r2_provider.get_public_url(clean_key)
        if self.active_provider.provider_name == "supabase":
            url = self.supabase_provider.get_public_url(clean_key)
            if not url.startswith("/storage/"):
                return url
        if settings.cloudflare_r2_public_url:
            return f"{settings.cloudflare_r2_public_url.rstrip('/')}/{clean_key}"
        if settings.supabase_storage_public_url:
            return f"{settings.supabase_storage_public_url.rstrip('/')}/{clean_key}"
        return self.active_provider.get_public_url(clean_key)
