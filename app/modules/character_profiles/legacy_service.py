...
from app.modules.character_profiles.persistence.repositories import CharacterProfileRepository
...
def _merge_address_term_list(
    ...
    for existing in terms:
        ...
        if norm_existing == norm_target or clean_existing == clean_target:
            ...
            if new_other and new_other.lower() == ex_other.lower() and new_other != ex_other:
                # Event ordering can vary when timestamps tie; retain the
                # more human-readable casing for an otherwise identical term.
                preferred = max(
                    (ex_other, new_other),
                    key=lambda value: sum(character.isupper() for character in value),
                )
                if "other" in existing:
                    existing["other"] = preferred
                if "other_term" in existing:
                    existing["other_term"] = preferred
            elif new_other and new_other.lower() not in ex_other.lower():
...
class CharacterProfileService:
    ...
    def __init__(
        ...
        self.submissions_by_key: Dict[tuple[str, str, str], str] = {}
    ...
    def _hydrate_all_from_storage(self, force: bool = False) -> None:
                    ...
                    for sub in CharacterProfileRepository.list_all_submissions(session):
                        if sub.submission_id not in self.submissions:
                            ...
                            if sub.idempotency_key:
                                self.submissions_by_key[
                                    (sub.book_id, sub.edition_id, sub.idempotency_key)
                                ] = sub.submission_id
        ...
        if self.storage_repo and getattr(self.storage_repo, "is_blob_active", False):
            try:
                all_r2_items = (
                    self.storage_repo.active_provider.list_json_objects("novels/")
                    + self.storage_repo.active_provider.list_json_objects("data/profile_books/")
                    + self.storage_repo.active_provider.list_json_objects("data/profile_editions/")
                    + self.storage_repo.active_provider.list_json_objects("data/profile_events/")
                    + self.storage_repo.active_provider.list_json_objects("data/profile_submissions/")
                )
                for item in all_r2_items:
                    ...
                    # Submissions
                    elif "submission_id" in item and (
                        "content_fingerprint" in item or "input_text_fingerprint" in item
                    ):
    ...
    def get_edition(
        ...
        if edition:
            ...
            if create_if_missing and title and edition.book_id in self.books:
        ...
        if create_if_missing and title:
        ...
        if create_if_missing and content and len(content) > 50:
        ...
        if create_if_missing and (edition_id.startswith("edition-") or len(edition_id) >= 10):
    ...
    def submit(
        ...
        with self._lock:
            if edition_id not in self.editions or self.editions[edition_id].book_id != book_id:
                raise KeyError("edition_not_found")
            idempotency_scope = (book_id, edition_id, idempotency_key)
            existing_id = self.submissions_by_key.get(idempotency_scope)
            if existing_id:
                existing = self.submissions[existing_id]
                if (
                    existing.local_chapter_index != local_chapter_index
                    or existing.input_type != input_type
            ...
            self.submissions_by_key[idempotency_scope] = submission.submission_id
    ...
    def process_candidates(
        ...
        with self._lock:
            ...
            try:
                ...
                processed = self._processed_chapters.setdefault(submission.book_id, set())
                processed.update(
                    range(
                        submission.canonical_chapter_start,
                        submission.canonical_chapter_end + 1,
                    )
                )
            except Exception as exc:
    ...
    def _character_id(self, book_id: str, candidate: CharacterEventCandidate, chapter: Optional[int] = None) -> str:
        ...
        for event in self._approved_events(book_id, chapter):
            ...
            if event.category == "identity" and event.attribute_key == "profile" and isinstance(event.value, dict):
                event_vi_name = _norm(event.value.get("vi_name"))
                event_original_name = _norm(event.character_original_name)
                candidate_vi_name = _norm(candidate.value.get("vi_name")) if isinstance(candidate.value, dict) else ""
                candidate_original_name = _norm(candidate.character_original_name)
                if (
                    (event_vi_name and candidate_vi_name and event_vi_name == candidate_vi_name)
                    or (event_vi_name and candidate_original_name and event_vi_name == candidate_original_name)
                    or (candidate_vi_name and event_original_name and candidate_vi_name == event_original_name)
                ):
    ...
    def delete_book(self, book_id: str) -> bool:
        ...
        with self._lock:
            ...
            if settings.structured_storage_backend in ("dual", "postgres"):
                try:
                    with db_session() as session:
                        deleted = CharacterProfileRepository.delete_book(session, book_id)
                        if not deleted and settings.structured_storage_backend == "postgres":
                            return False
                        session.commit()
                except Exception as exc:
                    if settings.structured_storage_backend == "postgres":
            ...
            if self.storage_repo and getattr(self.storage_repo, "is_blob_active", False) and settings.structured_storage_backend in ("legacy", "dual"):
                try:
                    keys = self.storage_repo.list_files(f"novels/{book_id}/profile/")
                    if keys:
                        self.storage_repo.delete_files(keys)
                except Exception as exc:
                    logger.warning("Failed to delete book profile from storage: %s", exc)
    ...
    def merge_books(self, source_book_id: str, target_book_id: str) -> bool:
        ...
        with self._lock:
            if source_book_id == target_book_id:
                return False
            if source_book_id not in self.books or target_book_id not in self.books:
                return False
            ...
            if settings.structured_storage_backend in ("dual", "postgres") and not db_success:
                return False

            source_events = [
                event for event in self.events.values() if event.book_id == source_book_id
            ]
            target_character_ids = {
                _norm(event.character_original_name): event.character_id
                for event in self.events.values()
                if event.book_id == target_book_id
            }
            character_id_map: Dict[str, str] = {}
            for event in source_events:
                target_character_ids.setdefault(
                    _norm(event.character_original_name),
                    self._character_id(
                        target_book_id,
                        CharacterEventCandidate(
                            character_original_name=event.character_original_name,
                ...
                character_id_map[event.character_id] = target_character_ids[
                    _norm(event.character_original_name)
                ]
            ...
            for ev in self.events.values():
                if ev.book_id == source_book_id:
                    ...
                    ev.character_id = character_id_map.get(ev.character_id, ev.character_id)
                    if isinstance(ev.value, dict):
                        for field in ("target_character_id", "source_character_id"):
                            if ev.value.get(field) in character_id_map:
                                ev.value[field] = character_id_map[ev.value[field]]
                    self._persist("profile_events", ev.event_id, ev)

            # Rebuild keys after the book/character namespace changes. Event
            # keys are derived from book identity and must not retain the old
            # source namespace after a merge.
            evidence_event_ids = {
                evidence.evidence_id: self._event_keys.get(evidence.event_key)
                for evidence in self.evidence.values()
            }
            self._event_keys.clear()
            self._event_evidence_groups.clear()
            for ev in self.events.values():
                candidate = CharacterEventCandidate(
                    character_original_name=ev.character_original_name,
                    character_id=ev.character_id,
                ...
                key = self._event_key(
                    ev.book_id, ev.character_id, ev.canonical_chapter, candidate
                )
                self._event_keys[key] = ev.event_id
                self._event_evidence_groups.setdefault(key, set()).add(ev.source_group_id)
                self._persist("profile_events", ev.event_id, ev, event_key=key)

            for evidence in self.evidence.values():
                event_id = evidence_event_ids.get(evidence.evidence_id)
                if event_id and event_id in self.events:
                    ...
                    evidence.event_key = self._event_key(
                        event.book_id, event.character_id, event.canonical_chapter, candidate
                    )
                    self._persist(
                        "profile_evidence",
    ...
    def _apply_event(states: Dict[str, CharacterSnapshot], event: CharacterEvent) -> None:
        ...
        if cat == "identity" and op == "link" and isinstance(val, dict):
            ...
            if not target_id and val.get("target_original_name"):
                target_key = f"{event.book_id}:{_norm(str(val['target_original_name']))}"
                target_id = f"char-{_hash(target_key, 24)}"
            if target_id:
        ...
        if op in {"set", "correct"}:
            if (
                ...
                for profile_key, profile_value in val.items():
                    if profile_key == "aliases" and isinstance(profile_value, list):
                        current_aliases = current_profile.setdefault("aliases", [])
                        for alias in profile_value:
                            if alias and alias not in current_aliases:
                                current_aliases.append(alias)
                    elif profile_key == "voice_notes" and profile_value:
                        current_voice_notes = current_profile.get("voice_notes")
                        if current_voice_notes and profile_value not in current_voice_notes:
                            current_profile["voice_notes"] = f"{current_voice_notes}; {profile_value}"
                        elif not current_voice_notes:
                            current_profile["voice_notes"] = profile_value
                    elif profile_value or profile_key not in current_profile:
                        current_profile[profile_key] = deepcopy(profile_value)
            else:
    ...
    def _merge_duplicate_snapshots(states: Dict[str, CharacterSnapshot]) -> List[CharacterSnapshot]:
        """Merge snapshots that refer to the same character under different names."""
        ...
        for snapshot in snapshots:
            profile = snapshot.attributes.get("profile", {})
            profile = profile if isinstance(profile, dict) else {}
            vi_name = _norm(profile.get("vi_name"))
            original_name = _norm(snapshot.original_name)
            aliases = {_norm(alias) for alias in profile.get("aliases", []) if alias}
            match = None

            for existing in merged:
                existing_profile = existing.attributes.get("profile", {})
                existing_profile = existing_profile if isinstance(existing_profile, dict) else {}
                existing_vi_name = _norm(existing_profile.get("vi_name"))
                existing_original_name = _norm(existing.original_name)
                existing_aliases = {
                    _norm(alias) for alias in existing_profile.get("aliases", []) if alias
                }
                if (
                    (vi_name and existing_vi_name and vi_name == existing_vi_name)
                    or (original_name and existing_original_name and original_name == existing_original_name)
                    or original_name in {existing_vi_name, *existing_aliases}
                    or existing_original_name in {vi_name, *aliases}
                    or vi_name in {existing_original_name, *existing_aliases}
                    or existing_vi_name in {original_name, *aliases}
                ):
                    match = existing
                    break

            if match is None:
                merged.append(snapshot.model_copy(deep=True))
                continue

            existing = match
            existing_profile = existing.attributes.setdefault("profile", {})
            if not isinstance(existing_profile, dict):
                existing_profile = {}
                existing.attributes["profile"] = existing_profile

            if _is_cjk(snapshot.original_name) and not _is_cjk(existing.original_name):
                old_name = existing.original_name
                existing.original_name = snapshot.original_name
                existing.character_id = snapshot.character_id
                current_aliases = existing_profile.setdefault("aliases", [])
                if old_name and old_name not in current_aliases and old_name != existing.original_name:
                    current_aliases.append(old_name)
            elif not _is_cjk(snapshot.original_name) and _is_cjk(existing.original_name):
                current_aliases = existing_profile.setdefault("aliases", [])
                if snapshot.original_name and snapshot.original_name not in current_aliases:
                    current_aliases.append(snapshot.original_name)
            elif not existing.original_name and snapshot.original_name:
                existing.original_name = snapshot.original_name

            if profile.get("vi_name") and not existing_profile.get("vi_name"):
                existing_profile["vi_name"] = profile["vi_name"]
            if profile.get("role") and not existing_profile.get("role"):
                existing_profile["role"] = profile["role"]
            if profile.get("voice_notes"):
                current_voice_notes = existing_profile.get("voice_notes")
                if current_voice_notes and profile["voice_notes"] not in current_voice_notes:
                    existing_profile["voice_notes"] = f"{current_voice_notes}; {profile['voice_notes']}"
                elif not current_voice_notes:
                    existing_profile["voice_notes"] = profile["voice_notes"]
            current_aliases = existing_profile.setdefault("aliases", [])
            for alias in profile.get("aliases", []):
                if alias and alias not in current_aliases and alias != existing.original_name:
                    current_aliases.append(alias)

            for attr_key, attr_value in snapshot.attributes.items():
                if attr_key == "profile":
                    continue
                if attr_key not in existing.attributes:
                    existing.attributes[attr_key] = deepcopy(attr_value)
                elif isinstance(existing.attributes[attr_key], list) and isinstance(attr_value, list):
                    for item in attr_value:
                        if item not in existing.attributes[attr_key]:
                            existing.attributes[attr_key].append(deepcopy(item))
                elif isinstance(existing.attributes[attr_key], dict) and isinstance(attr_value, dict):
                    for key, value in attr_value.items():
                        if key not in existing.attributes[attr_key] or not existing.attributes[attr_key][key]:
                            existing.attributes[attr_key][key] = deepcopy(value)

            chapters = [
                chapter
                for chapter in (existing.last_changed_chapter, snapshot.last_changed_chapter)
    ...
    def snapshot(
        ...
        with self._lock:
            ...
            processed = set(self._processed_chapters.get(edition.book_id, set()))
            for submission in self.submissions.values():
                if submission.book_id == edition.book_id and submission.status == "completed":
                    processed.update(
                        range(
                            submission.canonical_chapter_start,
                            submission.canonical_chapter_end + 1,
            ...
            complete_through = self._contiguous_complete_through(processed, through)
            pending = sorted(
    ...
    @staticmethod
    def _contiguous_complete_through(processed: set[int], through: int) -> Optional[int]:
        """Return the highest contiguous completed chapter, never a later hole."""
        if not processed:
            return None
        current = 0 if 0 in processed else 1
        if current not in processed:
            return None
        while current + 1 <= through and current + 1 in processed:
            current += 1
        return min(current, through)
