...
from app.modules.character_profiles.persistence.legacy_models import (
    ProfileBookModel,
    ProfileEditionModel,
...
class CharacterProfileRepository:
    ...
    def get_submission_by_idempotency_key(
        ...
        if book_id is not None:
            stmt = stmt.where(ProfileSubmissionModel.book_id == book_id)
        if edition_id is not None:
            stmt = stmt.where(ProfileSubmissionModel.edition_id == edition_id)
        model = session.execute(stmt).scalar_one_or_none()
        if not model:
            return None
        return cls._model_to_submission(model)
