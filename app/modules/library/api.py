...
async def create_novel_endpoint(
    ...
    cover_filename = None
    if cover:
        cover_data = await read_upload_limited(cover)
...
async def import_epub_endpoint(
    ...
    epub_bytes = await read_upload_limited(file)
    try:
...
def update_novel_endpoint(novel_id: str, req: NovelUpdateRequest, _: None = Depends(require_write_access)):
...
def delete_novel_endpoint(novel_id: str, _: None = Depends(require_write_access)):
