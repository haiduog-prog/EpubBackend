...
class HybridPolicyEngine:
    ...
    def apply_delta(
        ...
        existing_by_name = {
            BookBibleService._key(character.original_name): character
            for character in bible.characters
        }
        bible = BookBibleService.merge_delta(bible, delta)
        BookBibleService.ensure_timeline(bible)
        pending_ids: list[str] = []

        for new_char in delta.new_characters:
            old = existing_by_name.get(BookBibleService._key(new_char.original_name))
            if old and old.vi_name and new_char.vi_name and old.vi_name != new_char.vi_name:
                change_id = hashlib.sha256(
                    f"canonical:{old.character_id}:{new_char.vi_name}".encode("utf-8")
                ).hexdigest()[:24]
                if not any(item.change_id == change_id for item in bible.pending_changes):
                    bible.pending_changes.append(
                        PendingBibleChange(
                            ...
                            target_id=old.character_id,
                            old_value=old.vi_name,
                            proposed_value=new_char.vi_name,
                            evidence=f"chapter={chapter_index}",
                            confidence=0.0,
